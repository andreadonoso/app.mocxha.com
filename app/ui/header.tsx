...
export default function Header() {
  ...
  return (
    ...
    >
      <nav
        aria-label="Global"
        className={`select-none flex items-center justify-between px-6 xl:mx-4 xl:py-2 xl:px-3 py-6 xl:bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 xl:rounded-lg `}
      >
        ...
        <div className="hidden xl:flex xl:gap-x-1">
          {navigation.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-base/6 font-medium text-[gray-900] hover:bg-[#ffffff]/50 rounded-md px-3.5 py-2.5"
            >
              {item.name}
            </a>
          ))}
